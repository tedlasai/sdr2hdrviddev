"""
Benchmark WanVideoVAEMergeDecoderDeepHDR: forward-pass time, peak GPU memory,
and parameter count, for a single 704x1280x3 image merged from 3 exposures.
"""
import sys
...
model = WanVideoVAEMergeDecoderDeepHDR().to(DEVICE).eval()
...
with torch.no_grad():
    ...
    with torch.no_grad():
        for _ in range(NUM_ITERS):
            ...
            out = model(videos, exposures)
            if DEVICE.startswith("cuda"):
                torch.cuda.synchronize(DEVICE)
            times.append(time.perf_counter() - start)

times = torch.tensor(times)
print(f"\nInput: 3 exposures, each {B}x{C}x{T}x{H}x{W} on {DEVICE}")
