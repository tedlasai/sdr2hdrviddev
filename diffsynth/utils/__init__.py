...
class BasePipeline(torch.nn.Module):
    ...
    def preprocess_image(self, image, torch_dtype=None, device=None, pattern="B C H W", min_value=-1, max_value=1, in_max_value=255):
        ...
        image = image * ((max_value - min_value) / in_max_value) + min_value
        image = repeat(image, f"H W C -> {pattern}", **({"B": 1} if "B" in pattern else {}))
        return image


    def preprocess_video(self, video, torch_dtype=None, device=None, pattern="B C T H W", min_value=-1, max_value=1, in_max_value=255):
        # Transform a list of PIL.Image to torch.Tensor
        #if video is a list of PIL.Image
        if isinstance(video, list) or isinstance(video, np.ndarray):
            video = [self.preprocess_image(image, torch_dtype=torch_dtype, device=device, min_value=min_value, max_value=max_value, in_max_value=in_max_value) for image in video]
