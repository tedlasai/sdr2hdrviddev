...
def load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device):
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        #do a check if I am missing parameters
        ...
        if missing_params:
            model = model_class(**extra_kwargs) #initialize again but on cpu
            with torch.no_grad():
                # Check all parameters
                for name, param in model.named_parameters():
                    if name not in model_state_dict:
                        print(f"Initializing missing parameter: {name}")
                        model_state_dict[name] = param.detach().cpu().clone()
                # Check all buffers (e.g., running_mean, running_var, etc.)
                for name, buf in model.named_buffers():
                    if name not in model_state_dict:
                        print(f"Initializing missing buffer: {name}")
                        model_state_dict[name] = buf.detach().cpu().clone()
