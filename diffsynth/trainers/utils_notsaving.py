...
class ModelLogger:
    ...
    def on_epoch_end(self, accelerator, model, optimizer, scheduler, epoch_id):
        ...
        if accelerator.is_main_process:
            ...
            for old_f in model_files[:-2]:
                old_epoch = int(old_f.split("-")[1].split(".")[0])
                ...
                try:
                    os.remove(os.path.join(self.output_path, f"epoch-{old_epoch}.optimizer_scheduler.pt"))
                except FileNotFoundError:
                    pass
...
def validate(val_dataloader, model, model_logger, epoch_id, accelerator, dataset, args):
    ...
    for eval_mode, viz_exposures in eval_modes:
        if hasattr(dataset, 'set_predict_mode'):
            dataset.set_predict_mode(eval_mode)

        out_metrics, data_gathered, outputs_gathered = None, None, None
        saves = 0

        for step, data in enumerate(tqdm(val_dataloader, desc=f"Validation [{eval_mode}]", disable=not accelerator.is_local_main_process)):
            if skip_val:
                break
        ...
        del data_gathered, outputs_gathered, out_metrics
...
def launch_training_task(
    ...
    for epoch_id in range(epochs_done, num_epochs):
        with torch.no_grad():
            validate(val_dataloader, model, model_logger, epoch_id, accelerator, val_dataset, args)
        ...
        for data in tqdm(dataloader, desc=f"Epoch {epoch_id+1}/{num_epochs}"):
            with accelerator.accumulate(model):
                ...
                #     #if p name contains "dit gate", print the grad
                #     if 
        ...
        if save_steps is None:
            model_logger.on_epoch_end(accelerator, model, optimizer, scheduler, epoch_id)
