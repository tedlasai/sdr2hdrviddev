...
def launch_training_task(
    ...
    for epoch_id in range(epochs_done, num_epochs):
        ...
        with torch.no_grad():
            ...
            temp_pipeline = WanVideoPipeline(
                       dit = unwrapped_model.pipe.dit.to(accelerator.device),
                       vae = unwrapped_model.pipe.vae.to(accelerator.device),
                       image_encoder = unwrapped_model.pipe.image_encoder,
                       text_encoder= unwrapped_model.pipe.text_encoder.to(accelerator.device),
                       merge_decoder= unwrapped_model.pipe.merge_decoder.to(accelerator.device),
                       prompter = unwrapped_model.pipe.prompter
            ).to(accelerator.device)
            temp_pipeline.eval()

            device_type = "cuda" if torch.cuda.is_available() else "cpu"
            with torch.autocast(device_type, enabled=(accelerator.mixed_precision == "fp16")):
                for step, data in enumerate(tqdm(val_dataloader, desc="Validation", disable=not accelerator.is_local_main_process)):
                    ...
                    if  accelerator.is_main_process:
                        for i in range(len(data_gathered)):
                            ...
                            gt_combined_video = torch.from_numpy(data["video"]).unsqueeze(0).to(device)/255 #This scale feels funky here
